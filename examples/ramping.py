...
M = 5
N = 1
beta = np.array([-0.005,-0.0025,0.0,0.005,0.01])
...
beta = np.sort(npr.randn(5)*0.005)
log_sigma_scale = np.log(1e-3+2.5e-3*npr.rand())
...
q_elbos, q_lem = test_ramp.fit(ys, inputs=us, method="laplace_em",
								  ...
								  alpha=0.0, num_iters=0, initialize=False)
q_lem_init = copy.deepcopy(q_lem)
test_ramp_init = copy.deepcopy(test_ramp)

from ssm import hmm
arhmm = hmm.HMM(K, 1, M,
				init_state_distn=copy.deepcopy(test_ramp_init.init_state_distn),
				transitions=copy.deepcopy(test_ramp_init.transitions),
				observations=copy.deepcopy(test_ramp_init.dynamics))

arhmm.fit(q_lem_init.mean_continuous_states, inputs=us,
			method="em", num_iters=25)
...
q_elbos, q_lem = test_ramp.fit(ys, inputs=us, method="laplace_em",
								  ...
								  alpha=0.5, num_iters=25, initialize=False, num_samples=3)
plt.ion()
plt.figure()
plt.plot(q_elbos[1:])
plt.plot()
