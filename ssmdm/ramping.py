...
class RampingObservations(AutoRegressiveDiagonalNoiseObservations):
    def __init__(self, K, D=1, M=5, lags=1, beta=np.linspace(-0.02,0.02,5), log_sigma_scale=np.log(np.log(100)), x0=0.4):
        assert K == 2
        assert D == 1
        assert M == 5
        ...
        self._log_sigmasq[0] = np.log(1e-4) + np.exp(self.log_sigma_scale)
        self._log_sigmasq[1] = np.log(1e-4)

        # set init params
        self.mu_init = self.x0 * np.ones((2,1))
        self._log_sigmasq_init[0] = np.log(1e-4) + np.exp(self.log_sigma_scale)
        self._log_sigmasq_init[1] = np.log(1e-4)
    ...
    def params(self, value):
        ...
        self._log_sigmasq = np.log(1e-4)*np.ones((2,1)) + sig_mask * np.exp(self.log_sigma_scale)
        self._log_sigmasq_init = np.log(1e-4)*np.ones((2,1)) + sig_mask * np.exp(self.log_sigma_scale)
        self.mu_init = self.x0 * np.ones((2,1))
...
#         yhat = self.link(np.clip(data, .1, np.inf))
        ...
        if np.abs(xhat[0])>1.0:
                xhat[0] = 0.2*npr.randn(1,np.shape(xhat)[1])
        return xhat
...
class Ramping(SLDS):
    def __init__(self, N, K=2, D=1, *, M=5, beta=np.linspace(-0.02,0.02,5), log_sigma_scale=np.log(np.log(10)), x0=0.4, link="softplus", bin_size=1.0):
